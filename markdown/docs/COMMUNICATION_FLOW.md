...
3.  **同意要求 (Agent → Manager):** EULAが未同意の場合、`Agent`はサーバーを起動しません。この状態はエラーとして扱われず、代わりに`REQUIRE_EULA_AGREEMENT`メッセージを`Manager`に返します。`payload`には`eula.txt`の現在の内容が含まれます。
...
sequenceDiagram
    ...
    alt ユーザーが詳細画面を開いた場合
        ...
        Renderer->>Main: IPC: proxyToAgent('start-metrics-stream', ...)
        Main->>Agent: WebSocket: {type: 'START_METRICS_STREAM', ...}
    end

    alt サーバーが起動完了した場合 (詳細画面表示中)
        Agent-->>Main: WebSocket: {type: 'SERVER_UPDATE', payload: {status: 'running'}}
        Main-->>Renderer: IPC: 'server-update'
    ...
    loop 毎秒 (ストリーム実行中)
        ...
        Agent-->>Main: WebSocket: {type: 'GAME_SERVER_METRICS_UPDATE', payload: {...}}
        Main-->>Renderer: IPC: 'game-server-metrics-update'
        Renderer->>Renderer: 受信したデータでUIを更新
    end

    User->>Renderer: 一覧画面に戻る
    Renderer->>Main: IPC: proxyToAgent('stop-metrics-stream', ...)
    Main->>Agent: WebSocket: {type: 'STOP_METRICS_STREAM', ...}
...
sequenceDiagram
    ...
    Renderer->>Main: IPC: proxyToAgent('update-server-properties', {serverId, properties})
    Main->>Agent: WebSocket: {type: 'UPDATE_SERVER_PROPERTIES', requestId, payload: {serverId, properties}}
    Agent->>Agent: server.properties ファイルを更新
    Agent->>Agent: nl-server_manager.json を更新
    Agent-->>Main: WebSocket: {type: 'OPERATION_RESULT', requestId, success: true, payload: {serverId, properties}}
    Main-->>Renderer: IPC: 'operation-result'
    Renderer->>Renderer: stateを更新
    Renderer->>User: 「保存しました」通知を表示
```

1.  **UI → Main (IPC):** ユーザーが保存ボタンをクリックすると、レンダラーはUI上のすべてのプロパティ入力から値を取得し、`update-server-properties` という新しいメッセージタイプでメインプロセスに中継を依頼します。
2.  **Main → Agent (WebSocket):** メインプロセスは要求に `requestId` を付与し、`Agent` に `UPDATE_SERVER_PROPERTIES` メッセージを送信します。
3.  **Agentでの処理:** `Agent` は受信したプロパティで `server.properties` ファイルと `nl-server_manager.json` の両方をアトミックに更新します。
4.  **Agent → Main → UI (レスポンス):** `Agent` は処理結果を `OPERATION_RESULT` で返します。成功した場合、UIは状態を更新し、ユーザーに成功通知を表示します。
```

1.  **ストリーム開始要求:** メトリクスストリーミングは、以下のいずれかのタイミングで開始されます。
    -   ユーザーがサーバー詳細画面を開いたとき。
    -   ユーザーがサーバー詳細画面を閲覧中に、対象サーバーのステータスが「起動完了 (`running`)」に変化したとき。
    いずれの場合も、`Manager`は`START_METRICS_STREAM`メッセージを`Agent`に送信します。
2.  **メトリクス収集と送信:** `Agent`は、この要求を受け取ると、1秒ごとにメトリクス（TPS、プレイヤー数、CPU/RAM使用率など）を収集し、`GAME_SERVER_METRICS_UPDATE`または`PHYSICAL_SERVER_METRICS_UPDATE`メッセージで`Manager`に送信し続けます。
3.  **UIのリアルタイム更新:** `Manager`は受信したメトリクスデータを使って、UIの状態をリアルタイムで更新します。
4.  **ストリーム停止要求:** ユーザーが詳細画面を閉じると、`Manager`は`STOP_METRICS_STREAM`メッセージを`Agent`に送信し、`Agent`はメトリクスの収集と送信を停止します。